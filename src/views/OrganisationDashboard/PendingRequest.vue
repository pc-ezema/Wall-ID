                            ...
                            <div class="QA_section">
                                <div class="QA_table mb_30">
                                    <table class="table lms_table_active">
                                        ...
                                        <tbody v-if="!myDeclineRequest || !myDeclineRequest.length">
                                            <tr>
                                                <td class="align-enter text-dark font-13" colspan="6">No Decline Request.</td>
                                            </tr>
                                        </tbody>
                                        <tbody v-else>
                                            <tr v-for="(row, index) in myDeclineRequest" v-bind:key="index">
                                                ...
                                                <td>
                                                    <button @click="approveRequest(row.id)" class="tbl-btn btn-enable">Approve</button>
                            ...
                            <div class="QA_section">
                                <div class="QA_table mb_30">
                                    <table class="table lms_table_active">
                                        ...
                                        <tbody v-if="!myPendingRequest || !myPendingRequest.length">
                                            <tr>
                                                <td class="align-enter text-dark font-13" colspan="6">No Pending Request.</td>
                                            </tr>
                                        </tbody>
                                        <tbody v-else>
                                            <tr v-for="(row, index) in myPendingRequest" v-bind:key="index">
                                                <th scope="row">{{ index + 1 }}</th>
                                                <td>{{ row.name }}</td>
                                                <td>{{ row.created_by_individual.phone }}</td>
                                                <td>{{ row.created_at }}</td>
                                                ...
                                                <td>
                                                    <button @click="declineRequest(row.id)" class="tbl-btn btn-disable">Decline</button>
                                                    <button @click="approveRequest(row.id)" class="tbl-btn btn-enable">Approve</button>
                                                    <!-- <button class="tbl-btn btn-delete">Delete</button> -->
                                                </td>
                                            </tr>
    ...
    methods: {
        onChange(event) {
            if(this.key == 'Pending') {
                ...
                axios.get('verificaton/organization/pending')
                .then(
                    response => {
            ...
            if(this.key == 'Declined') {
                ...
                axios.get('verificaton/organization/decline')
                .then(
                    response => {
                        this.prepPagination(response.data);
        ...
        async approveRequest(data){
            ...
            await axios.post('verificaton/organization/approve/' + data)
            .then(
                response => {
        ...
        async declineRequest(data){
            ...
            await axios.post('verificaton/organization/decline/' + data)
            .then(
                response => {
                    this.$Progress.finish();
                    this.$notify({
